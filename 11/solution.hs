...
horizontals g = concat $ map r g
	where 
	  r (x:x1:x2:[]) = [] 
	  r l = (lt4 $ take 4 l) : r (tail l)

-- zip values from each set of 4 consecutive rows
verticals :: [[a]] -> [(a,a,a,a)]
verticals g = concat $ s g
	where
	  s (r1:r2:r3:[]) = []
...
diagonals g = d g
	where
	  d (r1:r2:r3:[]) = []
	  d l@(r1:r2:r3:r4:rs) = (zip4 r1 (drop 1 r2) (drop 2 r3) (drop 3 r4)) 
                          ++ (zip4 (drop 3 r1) (drop 2 r2) (drop 1 r3) r4) 
                          ++ (d $ tail l)
...
main = do
    ...
    print $ horizontals grid
    putStrLn "Verticals"
    print $ verticals grid
    putStrLn "Diagonals"
    print $ diagonals grid

    print $ map calc allLines
    print "Maximum product"
    print $ maximum $ map calc allLines
